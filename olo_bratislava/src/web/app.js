...
const mobileServicesSelect = document.querySelector("#mobile-services");
...
function renderSvg(paths) {
  return `
    <svg viewBox="0 0 124 124" fill="currentColor" aria-hidden="true" class="waste-badge-svg">
      ${paths.map((path) => `<path d="${path}"></path>`).join("")}
    </svg>
  `;
}

function renderWasteIcons(wasteTypes, className) {
  return (Array.isArray(wasteTypes) ? wasteTypes : [])
    .map((key) => {
      const meta = WASTE_META[key] || WASTE_META.mixed;
      return `
        <span class="${className}" style="--badge-bg:${meta.bg};--badge-fg:${meta.fg}">
...
function formDataToSettings() {
  ...
  return {
    ...
    refreshIntervalHours: Number(data.get("refreshIntervalHours")),
    notificationTime: data.get("notificationTime"),
    showMixed: form.elements.showMixed.checked,
    showKitchenBio: form.elements.showKitchenBio.checked,
    showGardenBio: form.elements.showGardenBio.checked,
    showPlastic: form.elements.showPlastic.checked,
    showPaper: form.elements.showPaper.checked,
    showGlass: form.elements.showGlass.checked,
    notifyPersistent: form.elements.notifyPersistent.checked,
    notifyMobile: form.elements.notifyMobile.checked,
    mobileNotifyServices: Array.from(mobileServicesSelect.selectedOptions).map(
      (option) => option.value,
    ),
    locale: data.get("locale") || "sk",
  };
}

function applySettings(settings) {
  form.elements.sourceUrl.value = settings.sourceUrl || "";
  form.elements.registrationNumber.value = settings.registrationNumber || "";
  form.elements.refreshIntervalHours.value = settings.refreshIntervalHours || 12;
  form.elements.notificationTime.value = settings.notificationTime || "18:00";
  form.elements.showMixed.checked = !!settings.showMixed;
  form.elements.showKitchenBio.checked = !!settings.showKitchenBio;
  form.elements.showGardenBio.checked = !!settings.showGardenBio;
  form.elements.showPlastic.checked = !!settings.showPlastic;
  form.elements.showPaper.checked = !!settings.showPaper;
  form.elements.showGlass.checked = !!settings.showGlass;
  form.elements.notifyPersistent.checked = !!settings.notifyPersistent;
  form.elements.notifyMobile.checked = !!settings.notifyMobile;
  form.elements.locale.value = settings.locale || "sk";
  const selected = new Set(settings.mobileNotifyServices || []);
  Array.from(mobileServicesSelect.options).forEach((option) => {
    option.selected = selected.has(option.value);
  });
}

function renderPreview(snapshot) {
  const next = snapshot.entitySnapshot.upcoming[0];
  if (!next) {
    previewRoot.className = "preview-empty";
    previewRoot.textContent = "Zatial nie su k dispozicii ziadne data.";
    return;
  }
  previewRoot.className = "preview-card";
  previewRoot.innerHTML = `
    <div class="icon-grid">${renderWasteIcons(next.wasteTypes, "waste-icon")}</div>
    <div class="preview-meta">
      <div class="preview-day ${next.isTomorrow ? "warning" : ""}">${escapeHtml(next.dayLabel)}</div>
      <div class="preview-date">${escapeHtml(next.dateLabel)}</div>
      <div class="preview-subtext">${escapeHtml(next.wasteTitles.join(", "))}</div>
    </div>
  `;
}

function renderUpcoming(snapshot) {
  const items = snapshot.entitySnapshot.upcoming || [];
  if (!items.length) {
    upcomingList.innerHTML =
      '<div class="preview-empty">Zatial nie su k dispozicii ziadne terminy.</div>';
    return;
  }
    ...
    .map(
      (item) => `
        <div class="upcoming-row">
          <div class="upcoming-icons">${renderWasteIcons(item.wasteTypes, "mini-icon")}</div>
          <div>
            <div class="upcoming-label ${item.isTomorrow ? "warning" : ""}">${escapeHtml(item.dayLabel)}</div>
            <div class="upcoming-date">${escapeHtml(item.wasteTitles.join(", "))}</div>
          </div>
...
function renderStatus(snapshot) {
  const entity = snapshot.entitySnapshot;
  statusPill.textContent = entity.status;
  statusPill.style.background =
    entity.status === "ok"
      ? "rgba(15,118,110,0.12)"
      : entity.status === "stale_cache"
        ? "rgba(217,119,6,0.14)"
        : "rgba(185,28,28,0.14)";
  statusPill.style.color =
    entity.status === "ok" ? "#115e59" : entity.status === "stale_cache" ? "#b45309" : "#991b1b";
  statusText.textContent =
    entity.lastError ||
    (entity.address
      ? `Adresa: ${entity.address} | Posledna obnova: ${entity.lastRefresh || "n/a"}`
      : "Add-on je pripraveny, ale chyba konfiguracia.");
}

async function loadNotifyServices(selected) {
  const response = await fetch("./api/notify-services");
  const payload = await response.json();
  mobileServicesSelect.innerHTML = (payload.services || [])
...
async function loadSnapshot() {
  ...
  await loadNotifyServices(snapshot.settings.mobileNotifyServices);
...
saveButton.addEventListener("click", async () => {
  ...
  try {
    const response = await fetch("./api/settings", {
      ...
      body: JSON.stringify(formDataToSettings()),
    });
    const snapshot = await response.json();
    renderStatus(snapshot);
    renderPreview(snapshot);
